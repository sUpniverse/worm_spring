...
- implements InitializingBean, DisposableBean를 통해 생명주기 설정
  - public void afterPropertiesSet()  -> 객체.refresh() 과정에서 호출되는 부분
  - public void destroy() -> 객체.close() 의 과정에서 호출, 객체.destroy()도 가능
...
- Step
  ...
  1. ConfigurableApplicationContext ctx = new GenericXmlApplicationContext()

     - GenericXmlApplicationConetext()의 최상위 객체임..

  2. ConfigurableEnvironment env = ctx.getEnvironmnet()

     - ctx에 있는 env를 가져온다.

  3. MutablePropertySource propertySource = env.getPropertySource()

     - 가져온 Environment안에 있는 PropertySource를 가져온다.
     - set을 하지 않았기에 빈 객체이다.

  4. propertySource.addLast(new ResoucePropertySouce("classpath:admin.pro"))

     - resouces에 porperties를 설정해준 후  가져왔던 propertysource에 addLast라는 메소드를 이용해서 등록해준다. (List나 queue처럼 add하는 개념)

  5. GenericXmlApplicatonContext gCtx = (GenericXmlApplicatonContext)ctx
  ...
  6. AdminConnection adminConnection = gCtx.getbean("adminConnection,AdminConnection.class")
     ...
     - AdminConnection.java 파일을 통해 설정해 준다.
       ...
       - AfterPropertiesSet() 메소드가 자동으로 Override되며 메소드에 해당 하는 Adminid 와 Adminpassword를 설정 코드를 삽입해준다.
         ...
         - 똑같이 setter를 이용해 해당 field의 선언된 값을 env.getProperty("")를 

           이용하여 가져온후 set해준다.

           setAdminId(env.getProperty("admin.id")) 

       - 호출 순서 setEnvironment() -> aterPropertieSet ()
...
- XML을 이용한 설정
  ...
  - 다만 bean안에 property를 직접 선언하여 가져온다.
    ...
    1. <context:property-placeholder location="classpath:~~">을 사용하여 아까와 마찬가지로 만들어논 properties에 접근

       - 다만 xml 설정이 필요하므로 아래 Namespaces 탭을 이용하여 context에 체크를 하여 설정을 불러온다.

    2. <property name=" ">

       ​	<value> ${admin.id} </value> 를 통해 각 값들을 가져온다.
...
- Java파일을 이용한 설정
  ...
  - AnnotationConfigApplication ctx = new Annotaion~~(ApplicationConfig.class)
  ...
  - ApplicationConfig.java파일을 생성
    ...
    3. @Bean 
       ...
       - 잘보면 location변수로 선언한건 configurer.setLocation메소드 때문이다.
...
- xml
  ...
  - ctx.refresh()는 필수
  - 해당 xml의 schemaLocation에 profile="~~"을 설정해야 한다.

- java

  - xml과 Main은 거의 동일하며 ctx.getEnvironment까지는 동일
